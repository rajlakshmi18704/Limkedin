...
export const getFeedPosts=async(req,res)=>{
   try {
    ...
    .poulate("comments.user","name profilePicture").sort({createdAt:-1});
    res.status(200).json(posts)
   } catch (error) {
    console.log("Error in getFeed Controller",error)
...
const postId=req.params.id;
const post=await Post.findById(postId)    
.populate("author","name username profilePicture headline").populate("comments.user",
...
export const createComment=async(req,res)=>{
    try {
        ...
        const post=await Post.findByIdAndUpdate(postId,{
            $push:{comments:{user:req.user._id,content}},},{new:true}
