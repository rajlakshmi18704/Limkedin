...
export const acceptConnectionRequest = async (req, res) => {
    try {
      ...
      try{
        await sendConnectionRequestEmail(senderEmail,senderName,recipientName,profileUrl)
      }
    
      catch (error) {
        res.status(500).json({ message: "Error in sendConnection Accepted Email" });
    }
    }
        
        catch (error) {
            console.log("Error in sendConnection Accepted Email", error);
            res.status(500).json({ message: "Server error" });
        }
    }
...
console.log("Error in rejectConnectionRequestEmail",error)
res.status(500).json({message:"Server error"})
}}
export const getConnectionRequests = async (req, res) => {
    try {
        ...
        .populate("sender", "name username profilePicture connections headline") 
...
export const  getUserConnections = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).populate(
           "connections",   "name username profilePicture headline connections"
        );
